...
with st.sidebar:
    ...
    node_spacing = st.slider("Node Spacing", 0, 100, 40)
    node_thickness = st.slider("Node Thickness", 5, 50, 20)
    arrow_size = st.slider("Arrow Head Size", 0, 30, 15)
    
    # NEW: Node Alignment Toggle
    st.write("Node Alignment")
    align_ui = st.radio(
        "Alignment Options",
...
if sankey_input_text:
    try:
        ...
        fig = go.Figure(data=[go.Sankey(
            arrangement = "snap", # This helps nodes stay where you drag them slightly better
            ...
            node = dict(
                ...
                align = node_alignment # PASSED FROM SIDEBAR
            ),
            link = dict(
                source = src,
                target = tgt,
