...
with st.sidebar:
    ...
    # Flow & Node Settings
    node_opacity = st.slider("Link Opacity", 0.1, 1.0, 0.45, on_change=st.rerun)
    node_spacing = st.slider("Node Spacing", 0, 100, 40, on_change=st.rerun)
    node_thickness = st.slider("Node Thickness", 5, 50, 20, on_change=st.rerun)
    arrow_size = st.slider("Arrow Head Size", 0, 20, 10, on_change=st.rerun)
    ...
    fig_width = st.number_input("Figure Width (px)", min_value=300, max_value=3000, value=1400)
